...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/moeleo/crack-soga-v2ray/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/moeleo/crack-soga-v2ray/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/soga -N --no-check-certificate https://github.com/moeleo/crack-soga-v2ray/raw/master/soga.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_usage() {
    ...
    echo "soga version      - 查看 soga 版本"xiaoanweiwu
    echo "------------------------------------------"
}
...
--- https://github.com/xiaoanweiwu/crack-soga-v2ray ---
  ${green}0.${plain} 退出脚本
————————————————
